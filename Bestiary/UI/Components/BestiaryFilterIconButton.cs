...
{
    /// <summary>图鉴筛选钮：青绿底 + 漏斗图标（与搜索框同色系）。</summary>
    internal sealed class BestiaryFilterIconButton : UIElement
    {
        private const float UiScale = 0.8f;
        private const float OriginalOuterPixels = 28f * UiScale;

        private readonly OPJourneyUI _shell;
        private readonly Action _onClick;

        public static float OuterSize => OriginalOuterPixels * 2f;

        public BestiaryFilterIconButton(OPJourneyUI shell, Action onClick)
        {
            _shell = shell;
            _onClick = onClick;
            Width.Set(OuterSize, 0);
            Height.Set(OuterSize, 0);
            OnLeftClick += (_, _) => _onClick?.Invoke();
        }
        ...
        protected override void DrawSelf(SpriteBatch spriteBatch)
        {
            ...
            if (IsMouseHovering && Main.LocalPlayer != null)
                Main.instance.MouseText(EOPJText.UI("BestiaryFilterBtn"));
        }
    }
}
