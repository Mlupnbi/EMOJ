...
namespace EvenMoreOverpoweredJourney.FurnitureBlueprint
{
    ...
    public static class FurnitureCandidateFilter
    {
        ...
        public static bool PassesSlotRules(Item item, FurnitureSlotKind slot)
        {
            ...
            if (slot == FurnitureSlotKind.Platform && !TileID.Sets.Platforms[item.createTile])
                return false;
            ...
            if (slot == FurnitureSlotKind.Block)
            {
                if (TileID.Sets.Platforms[item.createTile])
                    return false;
                if (!Main.tileSolid[item.createTile] || Main.tileSolidTop[item.createTile])
                    return false;
            }
